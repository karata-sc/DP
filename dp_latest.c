...
int main(){
	...
	for(numa=1;numa<=100;numa++){
		...
		snprintf(tnuma,256,"%d",numa);
		strcat(snuma,tnuma);

	sprintf(filename_a,"city_mcepdata/city011/city011_%s.txt",snuma);
	strcpy(snuma,cleara);
	strcpy(tnuma,cleara);
	...
	for(i=0;fgets(buff2,256,fp)!=NULL;i++){
		if(i>2)
         	       fprintf(fp2,"%s",buff2);
		else if(i==1)
			strcpy(word_b[num-1],buff2);
	}
	...
	printf("num: %d\nword_a: %sword_b: %sdistance: %f\n\n",numa,word_a[numa-1],word_b[total_num-1],total_min);
